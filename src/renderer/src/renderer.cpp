...
std::vector<char> readShaderFile(const std::string& relativePath) {
    ...
    throw std::runtime_error("Failed to find shader: " + relativePath + " : " + std::filesystem::current_path().generic_string() + " => " + searchPaths[0].generic_string());
}
...
void RenderEngine::VulkanInternals::createDescriptorSetLayouts() {
    ...
    if (vkCreateDescriptorSetLayout(context.device, &layoutInfo, nullptr, &context.uniformDescriptorSetLayout) != VK_SUCCESS) {
...
void RenderEngine::VulkanInternals::createPipelines() {
    ...
    VkDescriptorSetLayout setLayouts[] = { context.uniformDescriptorSetLayout };
...
void RenderEngine::VulkanInternals::createUniformBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        createBuffer(context, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffers[i], uniformBuffersMemory[i]);
...
void RenderEngine::VulkanInternals::createDescriptorSets() {
    ...
    UBOAllocInfo.descriptorPool = context.descriptorPool;
...
bool RenderEngine::shouldWindowClose() {
    return glfwWindowShouldClose(internals->window);
}
...
VulkanContext& RenderEngine::getContext() const {
